...
namespace XNodeEditor
{
    ...
    public partial class NodeEditorWindow
    {
        ...
        {
            ///
            Texture2D gridTex = graphEditor.GetGridTexture256();
            var gridScale = graph.GridSnapStep / (float)gridTex.width;
            DrawGrid(rect, zoom, panOffset, gridTex, Vector2.zero, gridScale);

            ///
            Texture2D crossTex = graphEditor.GetSecondaryGridTexture();
            DrawGrid(rect, zoom, panOffset, crossTex, Vector2.one / 2f, graph.CrossGridScale);
        }

        private void DrawGrid(Rect rect, float zoom, Vector2 panOffset, Texture2D texture, Vector2 additionalTileOffset, float textureScale)
        {
            ...
            float xOffset = -(center.x * zoom + panOffset.x) / (texture.width * textureScale);
            float yOffset = ((center.y - rect.size.y) * zoom + panOffset.y) / (texture.height * textureScale);

            Vector2 tileOffset = new Vector2(xOffset, yOffset) + additionalTileOffset;

            // Amount of tiles
            float tileAmountX = Mathf.Round(rect.size.x * zoom) / (texture.width * textureScale);
            float tileAmountY = Mathf.Round(rect.size.y * zoom) / (texture.height * textureScale);
        ...
        public void DrawConnections()
        {
            ...
            foreach (XNode.Node node in graph.nodes)
            {
                ...
                foreach (XNode.NodePort output in node.Outputs)
                {
                    ...
                    Vector2 fromPos;
                    if (!graph.DrawStraightConnection || !nodeSizes.ContainsKey(output.node))
                    {
                        Rect fromRect;
                        if (!_portConnectionPoints.TryGetValue(output, out fromRect)) continue;
                        fromPos = fromRect.position;
                    }
                    else
                    {
                        fromPos = output.node.position + nodeSizes[output.node] * 0.5f;
                    }
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++)
                    {
                        ...
                        Vector2 toPos;
                        if (!graph.DrawStraightConnection || !nodeSizes.ContainsKey(input.node))
                        {
                            Rect toRect;
                        ...
                        {
                            toPos = input.node.position + nodeSizes[input.node] * 0.5f;
                        }
