        ...
        public void Controls()
        {
            ...
            switch (e.type)
            {
                ...
                case EventType.MouseDrag:
                    if (e.button == 0 && e.modifiers != EventModifiers.Alt)
                    {
                        if (IsDraggingPort)
                        {
                        ...
                        if (currentActivity == NodeActivity.DragNode)
                        {
                            ...
                            for (int i = 0; i < Selection.objects.Length; i++)
                            {
                                if (Selection.objects[i] is XNode.Node)
                                {
                                    ...
                                    if (gridSnap)
                                    {
                                        node.position.x = ((int)Mathf.Round((node.position.x) / graph.GridSnapStep)) * graph.GridSnapStep;
                                        node.position.y = ((int)Mathf.Round((node.position.y) / graph.GridSnapStep)) * graph.GridSnapStep;
                                    }
                    ...
                    else if (e.button == 1 || e.button == 2 || (e.button == 0 && e.modifiers == EventModifiers.Alt))
                    {
