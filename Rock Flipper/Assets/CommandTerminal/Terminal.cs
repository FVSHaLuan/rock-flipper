...
namespace CommandTerminal
{
    ...
    {
        private const float DesignedWidth = 1920.0f;
        private const float DesignedHeight = 1080.0f;
        ...
        private bool toggleImmediatelyFlag = false;
        private GameObject savedSelectedGameObject;

        private ObjectWithName blockInputLock = new ObjectWithName("Terminal");
        ...
        public void SetState(TerminalState new_state)
        {
            ...
            switch (new_state)
            {
                ...
                case TerminalState.OpenSmall:
                    {
                        open_target = DesignedHeight * MaxHeight * SmallTerminalRatio;
                ...
                default:
                    {
                        real_window_size = DesignedHeight * MaxHeight;
        ...
        private void DisableGameUIInput()
        {
            ///
            Entry.Instance.completeInputBlocker.AddBlockLock(blockInputLock);
        }

        private void EnableGameUIInput()
        {
            ///
            Entry.Instance.completeInputBlocker.RemoveBlockLock(blockInputLock);
        }
        ...
        {
            ///            
            var matrix = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(Screen.width / DesignedWidth, Screen.height / DesignedHeight, 1.0f));
            GUI.matrix = matrix;
        ...
        [ContextMenu("SetupWindow")]
        private void SetupWindow()
        {
            // real_window_size = Screen.height * MaxHeight / 3;
            real_window_size = DesignedHeight * MaxHeight / 3;
            window = new Rect(0, current_open_t - real_window_size, DesignedWidth, real_window_size);
            ...
            window_style = window_style == null ? new GUIStyle() : window_style;
        ...
        void DrawConsole(int Window2D)
        {
            ...
            if (ShowGUIButtons && GUILayout.Button("| run", input_style, GUILayout.Width(DesignedWidth / 10)))
            {
                EnterCommand();
            }
        ...
        void DrawGUIButtons()
        {
            ...
            float x_position = RightAlignButtons ? DesignedWidth - 7 * size : 0;
        ...
        void HandleOpenness()
        {
            ...
            ///
            if (toggleImmediatelyFlag)
            {
                toggleImmediatelyFlag = false;
        ...
        {
            ///
            if (string.IsNullOrWhiteSpace(command_text))
            {
                return;
            }

            ///
            if (command_text.Trim().ToUpper() == "CLEARHISTORY")
            {
                ///
                History.Clear();
            ...
            var args = Shell.RunCommand(command_text);
            ...
            ///
            if (args != null && args.Length > 0 && args[args.Length - 1].String == ".")
            {
                toggleImmediatelyFlag = true;
                SetState(TerminalState.Close);
            }
        }
        ...
        [RegisterCommand]
        private static void Fix(CommandArg[] commandArgs)
        {
            Instance.SetupWindow();
        }

        [RegisterCommand(Help = "float delay, string commands, string[] params")]
        private static void Rd(CommandArg[] commandArgs)
        {
            ///
            float delay = commandArgs[0].Float;

            ///
            string originalCommand = commandArgs[1].String;

            ///
            for (int i = 2; i < commandArgs.Length; i++)
            {
                originalCommand += " " + commandArgs[i].String;
            }

            ///
            Debug.LogWarningFormat("Executing command: {0} in {1} secs.", originalCommand, delay);

            ///
            Entry.Instance.executionHelper.StartCoroutine(ExecuteDelay(originalCommand, delay));
        }

        private static IEnumerator ExecuteDelay(string command, float delay)
        {
            ///
