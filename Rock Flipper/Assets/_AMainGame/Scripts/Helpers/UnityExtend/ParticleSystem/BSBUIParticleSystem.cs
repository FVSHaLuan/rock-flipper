...
[AddComponentMenu("BSB/UI/BSBUIParticleSystem")]
public class BSBUIParticleSystem : MaskableGraphic
{
    ...
    [Space]
    [SerializeField]
    private bool doNotSimulate = false;
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            // Size
            bool is3DSize = pSystem.main.startSize3D;
            Vector2 size = Vector2.one;
            if (!is3DSize)
            {
                size *= particle.GetCurrentSize(pSystem) * 0.5f;
            }
            else
            {
                size = particle.GetCurrentSize3D(pSystem) * 0.5f;
            }
            ...
            {
                // no rotation
                corner1.x = position.x - size.x;
                corner1.y = position.y - size.y;
                corner2.x = position.x + size.x;
                corner2.y = position.y + size.y;
    ...
    private float GetDeltaTime()
    {
        ...
        {
            return unscaledDeltaTime ? Time.fixedUnscaledDeltaTime : Time.fixedDeltaTime;
        }
    }
}
#endif
