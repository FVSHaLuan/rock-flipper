...
- `m_Name` — the node's unique id (`SkillGraphNode.NodeId` returns `name`). Must be unique across the whole graph (editor validates this and logs an error on duplicates during import). Convention: prefixed by tier/scope, e.g. `P0 Landing Cash`, `Unlock P1 Rock`, `Mouse Radius`.
