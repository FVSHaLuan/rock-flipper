...
public class Reporter : MonoBehaviour
{
    ...
    bool isGestureDone()
    {
        if (Entry.Instance == null || !Entry.Instance.GameSetting.enabledTerminal)
            return false;
...
#else
    // Old system fallback (your existing code)
    ...
    else
    {
        ...
        // --- rest of your original gesture analysis logic stays unchanged ---
        ...
        return gestureLength > 300f && gestureSum.magnitude < 100f;
    }
    ...
    bool isDoubleClickDone()
    {
        ...
        // We treat a "double click" as two quick presses within a short time window
        if (pointer.press.wasPressedThisFrame)
        {
            float t = Time.realtimeSinceStartup;

            if (t - lastClickTime < 0.3f) // same threshold you were using
            {
                lastClickTime = 0;
                return true;
            }
    ...
    Vector2 getDownPos()
    {
        ...
        if (pointer.press.wasPressedThisFrame)
        {
            downPos = pointer.position.ReadValue();
            return downPos;
        }
        return Vector2.zero;
#else
    ...
    void Update()
    {
        ...
        calculateStartIndex();
        // Won't show by gesture anymore, use terminal command to show
