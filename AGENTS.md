...
2. A general toolkit: most of the code here is written to be reused across the owner's *future* games, not just this one. When editing, default to asking "does this belong to Rock Flipper specifically, or to the reusable toolkit?" — see the folder-level `AGENTS.md` files for exactly where that line falls in each area.
...
- **`Assets/_AMainGame/`** — all of this game's specific content and scripts. See [Rock Flipper/Assets/_AMainGame/AGENTS.md](Rock%20Flipper/Assets/_AMainGame/AGENTS.md).
...
- `#if UNITY_EDITOR`, `#if !DISABLESTEAMWORKS`, and feature-branch defines (`BSB_VER_DEMO`, `BSB_VER_PLAYTEST`, `BSB_F2P`) gate large chunks of code throughout. See `Scripts/FeatureBranching/AGENTS.md`.
